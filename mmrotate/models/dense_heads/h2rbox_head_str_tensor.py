...
from .rotated_fcos_head import RotatedFCOSHead, INF
import math
...
class H2RBoxVHeadStuctureTensor(RotatedFCOSHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 ...
                 init_cfg=dict(
                     ...
                     override=dict(
                         ...
                         bias_prob=0.01)),
                 **kwargs):
        super(H2RBoxVHeadStuctureTensor, self).__init__(num_classes=num_classes,
    ...
    def loss(self,
        ...
        if len(pos_inds) > 0:
            ...
            if has_valid_aug:
                ...
                pos_angle_preds_aug = self.str_tensor_to_obb(pos_points_aug, pos_angle_preds_aug)[:, 4]

            pos_angle_preds = self.str_tensor_to_obb(pos_points, pos_angle_preds)[:, 4]
            
            if self.separate_angle:
                bbox_coder = self.h_bbox_coder
            else:
                bbox_coder = self.bbox_coder
                pos_bbox_preds = torch.cat([pos_bbox_preds, pos_angle_preds[..., None]],
                                           dim=-1)
                pos_bbox_targets = torch.cat(
                    [pos_bbox_targets, pos_angle_targets], dim=-1)
                if has_valid_aug:
                    pos_bbox_preds_aug = torch.cat(
                        [pos_bbox_preds_aug, pos_angle_preds_aug[..., None]], dim=-1)
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, angle_pred, centerness, points in zip(
            ...
            angle_pred = self.str_tensor_to_obb(points, angle_pred)[:, 4]
