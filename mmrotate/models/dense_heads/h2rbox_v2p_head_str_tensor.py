...
class H2RBoxV2PHeadStuctureTensor(RotatedAnchorFreeHead):
    ...
    def loss(self,
        ...
        if len(pos_inds) > 0:
            ...
            if len(pair_angle_preds):
                if ss_info[0] == 'rot':
                    # Rotate the predicted structure tensor by the angle applied to the original image
                    angle_ori_preds = self.rotate_str_tensor(angle_ori_preds, -torch.tensor(rot))
                    #d_ang = angle_trs_preds - angle_ori_preds - rot
                else:
                    # Flip the predicted structure tensor
                ...
                #d_ang[square_mask] = 0
                angle_ori_preds[square_mask,:] = 0
                angle_trs_preds[square_mask,:] = 0

                # Self-supervision loss between the pairs of predicted structure tensors
                loss_ss = self.loss_ss_symmetry(angle_ori_preds, angle_trs_preds)
                #loss_ss = self.loss_ss_symmetry(d_ang, torch.zeros_like(d_ang))
            else:
                # No self-supervision loss if no pair of bboxes
