...
class COBBFCOSHead(RotatedAnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 scale_angle=True,
                 angle_coder=dict(type='COBBCoder'), # FIXME this cannot be changed for now
    ...
    def _init_layers(self):
        ...
        out_dim_score = 4 # FIXME Hard coded for now
        self.fc_score = nn.Linear(self.in_channels, out_dim_score)

        out_dim_ratio = 1 # FIXME Hard coded for now
        self.fc_ratio = nn.Linear(self.in_channels, out_dim_ratio) # WEIRD probably error in the original code here. self.score_last_dim instead of self.ratio_last_dim
        nn.init.normal_(self.fc_score.weight, 0, 0.001)
        nn.init.constant_(self.fc_score.bias, 0)

        nn.init.normal_(self.fc_ratio.weight, 0, 0.001)
        nn.init.constant_(self.fc_ratio.bias, 0)

        self.scale_ratio = Scale(1.0)
    ...
    def forward_single(self, x, scale, stride):
        ...
        # WEIRD the original create a score_pred before this line
        # but it's overwritten here
        if self.score_type == 'cos':
            score_pred = torch.cos(self.fc_score(x).permute((0, 3, 1, 2))) * 0.5 + 0.5
        elif self.score_type == 'sigmoid':
            score_pred = torch.sigmoid(self.fc_score(x).permute((0, 3, 1, 2)))
        elif self.score_type == 'add0.5':
            score_pred = self.fc_score(x).permute((0, 3, 1, 2)) + 0.5
        elif self.score_type == 'softmax':
            score_pred = torch.softmax(self.fc_score(x).permute((0, 3, 1, 2)), dim=-1)
        else:
            score_pred = self.fc_score(x_score).permute((0, 3, 1, 2))

        if self.ratio_type == 'cos':
            ratio_pred = torch.cos(self.fc_ratio(x).permute((0, 3, 1, 2))) * 0.5 + 0.5
        elif self.ratio_type ==' sigmoid':
            ratio_pred = torch.sigmoid(self.fc_ratio(x).permute((0, 3, 1, 2)))
        elif self.ratio_type == 'add0.5':
            ratio_pred = self.fc_ratio(x).permute((0, 3, 1, 2)) + 0.5
        else:
            ratio_pred = self.fc_ratio(x).permute((0, 3, 1, 2))

        ratio_pred = self.scale_ratio(ratio_pred).float()

        return cls_score, bbox_pred, score_pred, ratio_pred, centerness


    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'score_preds', 'ratio_preds', 'centernesses'))
    def loss(self,
             cls_scores,
             bbox_preds,
             score_preds,
        ...
        Args:
            ...
            bbox_preds (list[Tensor]): Box energies / deltas for each scale
                ...
                num_points * 4.
            score_preds (list[Tensor]):
            ratio_preds (list[Tensor]):
        ...
        assert len(cls_scores) == len(bbox_preds) \
               == len(ratio_preds) == len(score_preds) == len(centernesses)
        ...
        flatten_score_preds = [
            score_pred.permute(0, 2, 3, 1).reshape(-1, 4) # FIXME
            for score_pred in score_preds
        ]
        flatten_ratio_preds = [
            ratio_pred.permute(0, 2, 3, 1).reshape(-1, 1) # FIXME
            for ratio_pred in ratio_preds
        ]
        ...
        flatten_score_preds = torch.cat(flatten_score_preds)
        ...
        pos_score_preds = flatten_score_preds[pos_inds]
        ...
        if len(pos_inds) > 0:
            ...
            if self.separate_angle:
                ...
                loss_ratio = self.loss_ratio(
                        pos_ratio_preds, pos_ratio_targets, avg_factor=num_pos)
    ...
    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'score_preds', 'ratio_preds', 'angle_preds', 'centernesses'))
    def get_bboxes(self,
                   cls_scores,
                   bbox_preds,
                   score_preds,
        ...
        for img_id in range(len(img_metas)):
            ...
            score_pred_list = [
                score_preds[i][img_id].detach() for i in range(num_levels)
            ]
            ratio_pred_list = [
                ratio_preds[i][img_id].detach() for i in range(num_levels)
            ]
    ...
    def _get_bboxes_single(self,
        ...
        mlvl_scores = []
        mlvl_centerness = []
        for cls_score, bbox_pred, score_pred, ratio_pred, centerness, points in zip(
                cls_scores, bbox_preds, score_preds, ratio_preds, centernesses,
                mlvl_points):
            ...
            score_pred = score_pred.permute(1, 2, 0).reshape(
                -1, 4) # FIXME
    ...
    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'score_preds', 'ratio_preds', 'centerness'))
    def refine_bboxes(self, cls_scores, bbox_preds, score_preds, ratio_preds, centernesses):
        assert False, "we should not be here" # FIXME -> angle or score/ratio here?
        ...
        for lvl in range(num_levels):
            ...
            score_pred = score_preds[lvl]
            ratio_pred = ratio_preds[lvl]
            bbox_pred = bbox_pred.permute(0, 2, 3, 1)
            bbox_pred = bbox_pred.reshape(num_imgs, -1, 4)
            score_pred = score_pred.permute(0, 2, 3, 1)
            score_pred = score_pred.reshape(num_imgs, -1, 4) # FIXME
            ratio_pred = ratio_pred.permute(0, 2, 3, 1)
            ratio_pred = ratio_pred.reshape(num_imgs, -1, 1) # FIXME
